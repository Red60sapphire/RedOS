...
workbox.routing.registerRoute(
	...
	async (event) => {
		...
		const basepath = "/red_files";
		let path = decodeURI(event.url.pathname);

		// Force Filer to be used in cache routes, as it does not require waiting for red to be connected
			...
			try {
				...
				return new Promise(async (resolve) => {
					...
					if (fetchResponse.ok) {
						...
						// to red's fs, causing them to be treated as
						// strings
...
function offlineError() {
	return `<!DOCTYPE html>
            ...
            <body>
            <div id="wrapper">
            <h1>RedOS is offline without offline support enabled.</h1>
