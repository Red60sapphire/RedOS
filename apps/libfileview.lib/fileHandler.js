...
const { ShortcutApp } = await red.import("red.globalscope");

const icons = await (await fetch(localPathToURL("icons.json"))).json();

export function openFile(path) {
	async function openApp(path) {
		const stat = await red.fs.promises.stat(path);
		if (stat.isDirectory()) {
			let manifest = await red.fs.promises.readFile(`${path}/manifest.json`);
			manifest = JSON.parse(manifest);
			if (red.apps[manifest.package]) {
				red.apps[manifest.package].open();
				return;
			}
			createFolderAppView(path, "app");
		} else {
			// Shortcut file
			const data = await red.fs.promises.readFile(path);
	...
	async function openImage(path, mimetype) {
		const data = await red.fs.promises.readFile(path);
		let fileView = red.wm.createGeneric("Image File");
	...
	async function openPDF(path) {
		const data = await red.fs.promises.readFile(path);
		let fileView = red.wm.createGeneric("PDF File");
		fileView.content.style.overflow = "auto";
	...
	async function openAudio(path, mimetype) {
		const data = await red.fs.promises.readFile(path);
		let fileView = red.wm.createGeneric("Audio File");
	...
	async function openVideo(path, mimetype) {
		const data = await red.fs.promises.readFile(path);
		let fileView = red.wm.createGeneric("Video File");
	...
	async function openHTML(path) {
		const data = await red.fs.promises.readFile(path);
		let fileView = red.wm.createGeneric("HTML Viewer");
	...
	switch (ext) {
		...
		case "css":
			...
			break;
		case "ajs":
			// red.processes.execute(path);
			const shell = red.settings.get("shell") || "/usr/bin/chimerix.ajs";
			red.settings.set("shell", shell);
			red.processes.execute(shell, ["--cmd", path], true);
			break;
		case "mp3":
	...
	if (ext === "app") {
		...
		if (stat.isDirectory()) {
			...
			const app = new ShortcutApp(
				path,
				JSON.parse(await red.fs.promises.readFile(path)),
			);
...
export async function getFileType(path) {
	switch (path.split(".").slice("-2").join(".")) {
		case "app.zip":
			return "Red App Archive";
		case "lib.zip":
			return "Red Library Archive";
		default:
			break;
	}
	...
	if (ext === "app" || ext === "lib") {
		...
		return `Red ${ext == "app" ? "Application" : "Library"}`;
	}
