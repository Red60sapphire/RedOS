const hterm = (await red.import("red.hterm")).default;
...
const config = red.settings.get("red-shell-config") || {};
red.settings.set("red-shell-config", config);
...
term.onTerminalReady = async () => {
	...
	term.setBackgroundColor(red.ui.theme.darkBackground);
	term.setCursorColor(red.ui.theme.foreground);

	if (red.settings.get("transparent-ashell")) {
	...
	const proc = await red.processes.execute(cmdline[0], cmdline.slice(1));
