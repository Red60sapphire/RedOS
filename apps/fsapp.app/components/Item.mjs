...
export function Item() {
	...
	if (this.type === "file") {
		this.icon = red.files.fallbackIcon;
		this.description = "Red File";
	} else {
		this.icon = red.files.folderIcon;
		this.description = "Folder";
	}
	this.mount = async () => {
		if (this.type === "dir" && this.ext !== "app" && this.ext !== "lib") return;
		try {
			const iconURL = await red.files.getIcon(this.absolutePath);
			this.icon = iconURL;
		} catch (e) {
			console.error(e);
		}
		try {
			const fileType = await red.files.getFileType(this.absolutePath);
