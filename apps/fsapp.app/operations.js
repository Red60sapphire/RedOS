	...
	if (selected.length === 1) {
		...
		if (fileSelected.getAttribute("data-type") === "file") {
			...
			if (
				...
				red.files.open(fileSelected.getAttribute("data-path"));
				return;
			}
...
async function newFolder(path) {
	if (!path) {
		let folderName = await red.dialog.prompt("Folder Name: ");
		if (folderName) {
			path =
				document
					.querySelector(".breadcrumbs")
...
async function newFile(path) {
	if (!path) {
		let fileName = await red.dialog.prompt("File Name: ");
		if (fileName) {
			path =
				document
					.querySelector(".breadcrumbs")
...
function upload() {
	...
	fauxput.onchange = async (e) => {
		const dialog = red.dialog.progress("Uploading files...");
...
async function deleteFile() {
	const dialog = red.dialog.progress("Deleting files...");
...
async function paste() {
	const path = document
		.querySelector(".breadcrumbs")
		.getAttribute("data-current-path");
	const dialog = red.dialog.progress("Pasting files...");
	if (removeAfterPaste) {
		// cut
...
async function rename() {
	...
	const filename = await red.dialog.prompt("Filename:");
	if (filename) {
		await fs.promises.rename(
