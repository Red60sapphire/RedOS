function RepoItem() {
	...
	return html`
		<div
			class=${[
				...
				use(state.currentRepo, (repo) =>
					(repo || Array(3))[0] === this.reponame ? "selected" : "inactive",
				),
			]}
			on:contextmenu=${(e) => {
				e.preventDefault();
		...
		>
			...
			<span bind:this=${use(this.repoNameElement)}>${use(this.reponame)}</span>
		</div>
	`;
}
...
export default function RepoList() {
	useChange(use(saved.repos), (repos) => {
		this.repos = repos.map(
			([name, url]) => html`<${RepoItem} reponame=${name} repourl=${url} />`,
		);
	});
			...
			<div id="repoScreen">
				<div id="repoList">
					...
					<div class="repoAdd">
						<button
							on:click=${async () => {
								let url = await red.dialog.prompt(
									"Enter the repo URL",
									"https://red.repo/",
								);
								var name;
								...
								if (!url.endsWith("/")) {
									// red.dialog.alert("URL does not end with a \"/\" character")
									// return;
									console.warn(
								...
								try {
									...
									name = await red.dialog.prompt(
										"Enter the repo name",
										"My awesome repo",
									);
								}

								if (saved.repos.filter(([n]) => n === url).length > 0) {
									red.dialog.alert("Repo is already added.");
									return;
								} else {
