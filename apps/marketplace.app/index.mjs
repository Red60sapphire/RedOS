...
const { Store } = await red.import("red.libstore@2.0.0");
const persist = await red.import("red.persistence");
const loader = persist.buildLoader(red);
await loader.locate();
const persistence = await loader.build(instance);

if (red.settings.get("workstore-repos")) {
	await persistence.set("repos", red.settings.get("workstore-repos"));
	red.settings.set("workstore-repos", null);
}

const branding = red.settings.get("marketplace-branding") || {
...
window.marketplace = new Store(red.net, {
...
// Example URIs:
// marketplace://add:Red%20App%20Repository:https%3A%2F%2Fraw.githubusercontent.com%2FMercuryWorkshop%2Fred-repo%2Fmaster%2F
// marketplace://remove:Red%20App%20Repository
// marketplace://view:repo:Red%20App%20Repository
// marketplace://view:app:Red%20App%20Repository:games.run3
// marketplace://view:lib:Red%20App%20Repository:red.flash.handler

if (!red.uri.has("marketplace")) {
	red.uri.set("marketplace", {
		handler: {
			tag: "app",
