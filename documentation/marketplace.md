...
To make a repo accessible via the Store Library you can use the utility [create-red-repo](https://github.com/MercuryWorkshop/create-red-repo). It's as simple as running a command in your terminal.
...
```js
let app = await repo.getApp("red.ide");
console.log("App Data: ", app);
```
...
```js
let icon = await repo.getAppThumb("red.ide");
...
```js
let lib = await repo.getlib("red.flash.handler");
console.log("Library Data: ", lib);
```
...
```js
await repo.installLib("red.flash.handler");
```
...
```js
let icon = await repo.getLibThumb("red.flash.handler");
let element = document.createElement("img");
element.src = icon;
document.body.appendChild(element);
```
