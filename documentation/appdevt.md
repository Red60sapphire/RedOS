# The Hitchhiker's Guide to RedOS App Development

See [this document](./templates/template.app/README.md) for instructions on how to set up a basic iFrame app.

## RedOS Apps

RedOS apps are simple creatures. They live inside folders with the suffix `.app` and the resources specific to each app are contained within that folder.
...
- `icon`: `String` - Path (from app directory) to the application's icon. Optional but highly recommended. Red will display this icon throughout the DE.
...
- `useIdbWrapper`: `Boolean` - Use the IndexedDB wrapper, which prevents the app from making accidental modifications to other app's indexeddb stores or red's own stores. Defaults to `false`. Optional.
...
- In iframed apps, Red still gives you full access to the APIs through the `red` object and also gives you access to your app instance and the Window in the Window Manager. You can access the WMWindow using `instanceWindow` and access your red app instance using `instance` in your javascript. This could be used to manipulate the window or invoke actions on your app instance. An example is shown below where a back arrow is drawn on the window decorations in the marketplace app.
...
- All Red windows allow you to hook into their events. This can be achieved via a callback or a event listener. You can see this in action in the window managment api and in multiple system apps.
...
if (instance.manifest.marketplace) {
	let libstore = await red.import("red.libstore@2.0.0");

	marketplace = new libstore.Store(red.net, {
...
RedOS itself uses dreamland for the desktop environment and core system apps, and you can use it in your apps as well. To include dreamland in your app without a bundler, you can add the following to the `head` section of your `index.html` file:

```html
<script src="/libs/dreamland/all.js"></script>
```

While dreamland itself includes a `$store` function for preserving state between page reloads, this function is not available by default. Instead, you can use the [`red.persistence`](#libpersist) library to build a `$store` function bound to your app instance.

```jsx
const { buildLoader } = await red.import("red.persistence");
const loader = buildLoader(red);
await loader.locate();
...
RedOS libraries are just like apps but contain utilities or functionality that other apps could import and use. They live inside folders with the suffix `.lib` and the resources specific to each app are contained within that folder.
...
- You write a library that consists of a `manifest.json` file and an ES module. An example of the manifest file is below.
  ...
  - `installHook` is a file that is run when the library is installed. It should have a default export that is a function that takes the red instance as an argument.
  - `currentVersion` is the current version of the library, which will be used as the default version when using the [`red.import`](./Red-API.md#redimport) api.

### Usage

- Libraries can be imported using the [`red.import`](./Red-API.md#redimport) api. This api takes the package id of the library and an optional version number. The version number is specified by appending `@<version>` to the package id. If no version is specified, the current version of the library is used.

```js
red.import("red.examplelib@1.0.0").then((lib) => {
	// Do stuff with the library.
});
```

```js
let lib = await red.import("red.examplelib");
// Do stuff with the library.
```

## System Libraries

Red has a assortment of preinstalled system libraries to streamline the developer experience. This part of the documentation outlines how to use them in your application using the [`red.import`](./Red-API.md#redimport) api.

### libbrowser

This library allows you to interact with Red's browser. Instead of using `window.open` in your red apps, using this API will ensure that the webpage will open up in the Red Browser.
...
This library allows you to select files from inside of red's filesystem. It returns a path(s) and supports both files and folders.

```js
let picker = await red.import("red.filepicker");
...
This library allows you to create and manage persistent data stores. It uses the red filesystem to store data.

The default storage backend uses the same format as the [`red.settings`](./Red-API.md#redsettings) api, except that it is stored in a file related to the app's data directory instead of in the root settings file. The persistence library also has the ability to be turned into a proxy object that will automatically save changes to the persistence store.

```js
let persist = await red.import("red.persistence");
let loader = persist.buildLoader(red);
await loader.locate();
...
Libraries can also be setup to handle files. A file handler library at the least requires a `openFile` function inside of the file handler, but can be extended. An example is shown below for a simple text editor that is integrated by default in red.

```js
export function openFile(path) {
	red.fs.readFile(path, async function (err, data) {
		let fileView = red.wm.createGeneric("Simple Text Editor");
...
After setting up a library like this you can make it the file handler for a file extension by just setting it using the `red.files.setModule` function.

```js
red.files.setModule("(package identifier)", "(file extension)");
```
