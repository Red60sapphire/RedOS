class Taskbar {
	timeformat = new Intl.DateTimeFormat(navigator.language, {
		hour: "numeric",
		minute: "numeric",
		hour12: !red.settings.get("sir-yes-sir"),
	});
	...
	state: {
		...
		activeApps: [],
		showBar: false,
		rounded: red.platform.type !== "mobile",
		time: "",
		date: "",
	...
	shortcut(app: App) {
		...
		return ((this as any).tmp = (
			<li class="taskbar-button">
				<input
					type="image"
					draggable={red.platform.type === "desktop"}
					src={app?.icon || ""}
					title={app?.name || "App"}
					on:dragend={(e: DragEvent) => {
						if (!this.insidedrag) {
							...
							let closestIndex = red.settings.get("applist").length - 1;
							...
							if (red.settings.get("applist").includes(app.package)) {
							...
							const order = [...red.settings.get("applist")];
							order.splice(closestIndex, 0, app.package);
							red.settings.set("applist", order);
	...
	#contextMenu = new ContextMenu(true); // This is going to be before red is initialized, so we can't use red.ContextMenu
	showcontext(app: App, e: MouseEvent) {
		...
		if (app.windows.length > 0 || e.button === 2) {
			...
			const pinned = red.settings.get("applist").includes(app.package);
			this.#contextMenu.addItem(
				pinned ? "Unpin" : "Pin",
				() => {
					if (pinned) {
	...
	constructor() {
		...
		document.addEventListener("red-force-taskbar-update", () => {
			this.updateTaskbar();
		});
						...
						>
							...
							<span>
								<span
									class={[
										"notification-badge",
										use(red.notifications.state.notifications.length, (i) =>
											i > 0 ? "shown" : "hidden",
										),
									]}
								>
									{use(red.notifications.state.notifications.length)}
								</span>
							</span>
						</div>
					</span>
	...
	updateTaskbar() {
		const pinned = red.settings
			.get("applist")
			.map((id: string) => red.apps[id]);
		const activewindows: App[] = Object.values(red.apps).filter(
	...
	updateRadius() {
		if (this.maximizedWins.length > 0 || snappedWindows.length > 0) {
			...
			if (red.platform.type !== "mobile" && red.platform.type !== "tablet")
