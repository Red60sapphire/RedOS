...
const SettingSwitch: Component<{
	...
	this.mount = () => {
		this.on = red.settings.get(this.setting);
	};
	return (
		<div class="settings-item">
			<label class="matter-switch">
				<input
					...
					role="switch"
					on:click={() => {
						red.settings.set(this.setting, this.on);
		...
		<div class="settings-item">
			...
			<input
				...
				on:change={(event: any) => {
					switch (this.type) {
						case "number":
							red.settings.set(this.setting, parseInt(event.target.value));
							break;
						case "string":
							red.settings.set(this.setting, event.target.value);
							break;
						default:
							red.settings.set(this.setting, event.target.value);
							break;
				...
				placeholder={red.settings.get(this.setting)}
				type="text"
			/>
		</div>
	);
...
class SettingsApp extends App {
	...
	state = $state({
		show_x86_install: red.settings.get("x86-disabled"),
			...
			<div css={this.state} class="container">
				...
				<div bind:this={use(this.state.settingsBody)} class="settings-body">
					<div id="general" class="general settings-category">
						...
						<div class="settings-group">
							<SettingSwitch
								...
								callback={async () => {
									if (
										await red.dialog.confirm(
											"This will restart Red. Are you sure?",
										)
									) {
										red.settings.set("milestone", "__INVALID");
										window.location.reload();
									} else {
										red.settings.set(
											"use-sw-cache",
											!red.settings.get("use-sw-cache"),
										);
									}
								}}
							/>
							...
							/>
							<SettingSwitch
								title="Transparent Red Shell Background"
					...
					<div id="v86" class="v86 settings-category">
						<h3 class="settings-category-name">Red x86</h3>
						<div class="settings-group">
								...
								<div>
									<button
										on:click={async () => {
											...
											red.settings.set("x86-image", "alpine");
											await installx86();
											red.settings.set("x86-disabled", false);
									...
									<div class="settings-item">
										<div class="disk-info">
											...
											<input
												...
												id="disk-size-bytes"
												value={Math.ceil(red.x86hdd.size / 1000000) || 0}
												type="text"
											/>
										</div>
										<button
											on:click={async () => {
												...
												clearInterval(red.x86?.saveinterval);
												...
												if (document.getElementById("disk-size-bytes")) {
													...
													const emulator = new V86({
														...
														screen_container: red.x86!.screen_container,
													...
													emulator.add_listener(
														...
														async (byte: number) => {
															...
															if (char === "\r") {
																red.logger.debug(s0data);

																if (s0data.includes("Finished Disk")) {
																	await red.x86hdd.save(emulator);
										...
										<button
											...
											on:click={async () => {
												...
												if (name && cmd) {
													...
													await red.registerApp(stub);
													red.settings.set("user-xapps", [
														...red.settings.get("user-xapps"),
														{
															name: name,
															cmd: cmd,
															id: stub.package,
													...
													red.dialog.alert("Please fill out both fields!");
												}
											}}
										>
											Register
									...
									<div class="settings-button-group">
										<button
											on:click={() => {
												...
												red.settings.set("x86-disabled", true);
												setTimeout(() => {
													red.x86hdd.delete();
												}, 200);
											}}
													...
													inp.addEventListener("change", async () => {
														if (inp.files) {
															if (inp.files[0]) {
																try {
																	// @ts-ignore
																	await red.x86hdd.loadfile(inp.files[0]);
																	red.notifications.add({
					...
					<div id="reset" class="settings-category">
						<h3 class="settings-category-name">Reset Red</h3>
						<div class="settings-group">
							<div class="settings-item">
								<span class="settings-item-name">
									Reset your Red install.
								</span>
								<button
									class="matter-button-outlined"
									on:click={async () => {
										const confirmation = await red.dialog.confirm(
											"Are you sure you want to powerwash Red? All of your data will be lost.",
										);
										if (confirmation) {
											try {
