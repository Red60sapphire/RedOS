...
class RegEdit extends App {
	...
	page = async () => (
		<div
			style={{
				...
				background: use(red.ui.theme.state.background),
			}}
				...
				<div id="detail">
					<details open>
						<summary
							class:selected={use(
								this.state.selected,
								(sel) => sel === red.settings.cache,
							)}
						>
							<span
								on:click={(e: MouseEvent) => {
									e.preventDefault();
									this.state.selected = red.settings.cache;
								}}
							>
								Root
							</span>
						</summary>
						{Object.entries(red.settings.cache)
							.filter((setting) => {
								return (
									setting[1] instanceof Object && !(setting[1] instanceof Array)
								);
												...
												<input
													...
													on:blur={function (event: any) {
														...
														try {
															...
															elements[2].value = red.settings.get(item[0]);
	...
	async open(): Promise<WMWindow | undefined> {
		if (
			!red.settings.get("disable-regedit-warning") &&
		...
		red.settings.set("disable-regedit-warning", true);
