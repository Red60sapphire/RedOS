...
class WallpaperAndStyle extends App {
			...
			<div class="header">
				...
				{$if(
					...
					>
						<button
							style={{
								color: use(red.ui.theme.state.foreground),
							}}
							on:click={() => {
								red.ui.theme.reset();
								red.settings.set("theme", red.ui.theme.state);
							}}
						>
						...
						<button
							...
							on:click={() => {
								this.exportTheme(JSON.stringify(red.ui.theme.state));
							}}
						>
								...
								<select
									...
									on:change={(e: Event) => {
										...
										window.document.body.style.backgroundSize =
											red.settings.get("wallpaper-fit");
									}}
								>
									<option
										value="cover"
										selected={
											...
											red.settings.get("wallpaper-fit") === "cover"
										}
									>
										Cover
									</option>
									<option
										value="contain"
										selected={
											red.settings.get("wallpaper-fit") === "contained"
										}
									>
										Contain
									</option>
									<option
										value="auto"
										selected={red.settings.get("wallpaper-fit") === "auto"}
									>
										Auto
									</option>
								</select>
								{$if(
									red.settings.get("wallpaper-fit") === "contain",
									<input
										type="color"
										name="contain-color"
										id="contain-color"
										value={red.settings.get("wallpaper-contain-color")}
										on:change={(e: Event) => {
											red.settings.set(
											...
											window.document.documentElement.style.backgroundColor =
												red.settings.get("wallpaper-contain-color");
										}}
									/>,
								)}
							</div>
				...
				<div id="colors">
					{this.colorEditors.map((color) => (
						<div class="editor">
							...
							<input
								type="color"
								bind:value={use(red.ui.theme.state[color.prop])}
								on:input={(e: InputEvent) => {
									const val = (e.target! as HTMLInputElement).value;
									red.ui.theme[color.prop] = val;
									red.settings.set("theme", val, color.prop);
								}}
							/>
						</div>
					))}
	...
	setNewWallpaper(wallpaperObj: WallpaperObject) {
		red.settings.set("wallpaper", wallpaperObj.url);
		red.settings.set("wallpaper-name", wallpaperObj.name);
	...
	getCurrentWallpaper(): WallpaperObject {
		let currWallpaper = red.settings.get("wallpaper");
		let currWallpaperName = red.settings.get("wallpaper-name");
		if (
			...
			red.settings.set("wallpaper", currWallpaper);
			red.settings.set("wallpaper-name", currWallpaperName);
		}
		return {
	...
	setWallpaper(url: string) {
		window.document.documentElement.style.backgroundColor = red.settings.get(
			"wallpaper-contain-color",
	...
	async importTheme() {
		...
		this.libfilepicker
			...
			.then(async (file: any) => {
				try {
					const data = await red.fs.promises.readFile(file);
					Object.assign(red.ui.theme.state, JSON.parse(data as any));
					red.ui.theme.apply();
					await red.settings.set("theme", red.ui.theme.state);
				} catch (e) {
	...
	exportTheme(theme: string) {
		...
		red.fs.writeFile(filePath, theme);
		red.notifications.add({
			title: "Theme editor",
	...
	async open(): Promise<WMWindow | undefined> {
		...
		if (this.libfilepicker === undefined) {
			...
			this.libfilepicker = await red.import("red.filepicker");
		}
