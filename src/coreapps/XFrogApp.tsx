		...
		for (const wid in this.xwindows) {
			if (!wids.includes(wid)) {
				...
				try {
					...
					red.logger.debug("Preclosed window " + e);
				}
		...
		const sfocus = async () => {
			...
			if (this.activeWin && this.activeWin !== win) {
				...
				const blob: any = await new Promise((resolve) =>
					red.x86?.vgacanvas.toBlob(resolve),
				)!;
			...
			timeout = setTimeout(() => {
				...
				for (const winRef in red.wm.windows) {
					try {
						if (red.wm.windows[winRef]!.deref() === win) {
							console.log(winRef);
						}
					} catch (e) {
			...
			(data) => {
				if (!win) {
					...
					if (Number(dimensions[0]) > 5 && Number(dimensions[1]) > 5) {
						...
						win.onclose = () => {
							red.x86!.runcmd(`DISPLAY=:0 xkill -id ${xwid}`);
						};
