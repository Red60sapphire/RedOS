...
abstract class RedFSOperations<TStats> {
	/*
 ...
 */
abstract class AFSProvider<TStats> extends RedFSOperations<TStats> {
	/**
...
class AFSShell {
	...
	cat(
		...
		files.forEach((file) => {
			red.fs.readFile(this.#relativeToAbsolute(file), (err, data) => {
				if (err) {
	...
	// This differs from the Filer version, because here we can use the red.files API to open the file
	...
	exec(path: string) {
		red.files.open(this.#relativeToAbsolute(path));
	}
	find(
		path: string,
		options?: {
	...
	find(
		...
		function walk(
			...
			red.fs.readdir(dir, (err: Error | null, list: string[]) => {
				if (err) {
					done(err, results);
					return;
				}
		...
		red.fs.stat(
			...
			(err: Error | null, stats: { isDirectory: () => boolean }) => {
				...
				if (options.recursive) {
					...
					red.fs.readdir(path, (err: Error | null, files: string[]) => {
						if (err) {
							callback!(err);
							return;
						}
	...
	touch(path: string, options?: any, callback?: (err: Error | null) => void) {
		...
		function createFile() {
			red.fs.writeFile(path, "", callback);
		}

		function updateTimes() {
			red.fs.utimes(path, options.date, options.date, callback);
		}

		red.fs.stat(path, (err: Error | null) => {
			if (err) {
	...
	promises = {
		cat: async (files: string[]) => {
			...
			for (const file of files) {
				contents += (
					await red.fs.promises.readFile(this.#relativeToAbsolute(file))
				).toString();
			}
			return contents;
		},
		...
		cpr: async (src: string, dest: string, options?: any) => {
			try {
				const stat = await red.fs.promises.stat(src);
				if (options?.createInnerFolder === true) {
					try {
						const destStat = await red.fs.promises.stat(dest);
				...
				if (stat.type === "FILE") {
					...
					await red.fs.promises.writeFile(
						dest,
						await red.fs.promises.readFile(src),
					);
				} else if (stat.type === "DIRECTORY") {
					await this.promises.mkdirp(dest);

					const items = await red.fs.promises.readdir(src);
		...
		mkdirp: async (path: string) => {
			...
			for (const part of parts) {
				...
				builder += "/" + part;
				try {
					await red.fs.promises.mkdir(builder);
				} catch (e) {
 ...
 * at arbitrary paths.
 */
class RedFilesystem implements RedFSOperations<any> {
	...
	whatwgfs = {
		...
		getFolder: async () => {
			...
			return await this.whatwgfs.fs.getOriginPrivateDirectory(
				// @ts-ignore
				import("/libs/nfsadapter/adapters/redadapter.js"),
			);
		},
		...
		directoryHandleFromPath: async (path: string) => {
			...
			// prettier-ignore
			let workingPath = (await red.fs.whatwgfs.getFolder());
		...
		fileHandleFromPath: async (givenPath: string) => {
			...
			// prettier-ignore
			const workingPath = (await red.fs.whatwgfs.directoryHandleFromPath(path));
			return await workingPath.getFileHandle(file);
		},
		showDirectoryPicker: async (options: object) => {
			const picker = await red.import("red.filepicker");
	...
	ftruncate(fd: number, len: number, callback?: (err: Error | null) => void) {
		red.fs.truncate(this.fds[fd].path, len, callback);
	}
	...
	fstat(
		...
		red.fs.stat(this.fds[fd].path, callback);
	}
	...
	async exists(path: string, callback?: (exists: boolean) => void) {
		try {
			await red.fs.promises.access(path);
			callback!(true);
		} catch (e) {
			callback!(false);
		}
	...
	futimes(fd: number, ...rest: any[]) {
		// @ts-ignore - Overloaded methods are scary
		red.fs.utimes(this.fds[fd].path, ...rest);
	}
	...
	fchown(fd: number, ...rest: any[]) {
		// @ts-ignore - Overloaded methods are scary
		red.fs.chown(this.fds[fd].path, ...rest);
	}
	...
	fchmod(fd: number, ...rest: any[]) {
		// @ts-ignore - Overloaded methods are scary
		red.fs.chmod(this.fds[fd].path, ...rest);
	}
		...
		try {
			...
			try {
				fileBuf = Filer.Buffer.from(
					(await red.fs.promises.readFile(
						realPath,
			...
			await red.fs.promises.writeFile(realPath, outBuf);
	...
	async read(fd: number, ...rest: any[]) {
		...
		try {
			...
			const fileBuf = await red.fs.promises.readFile(realPath);
	...
	promises = {
		...
		open: async (
			...
			if (["a", "a+", "w", "w+"].includes(flags)) {
				red.fs.promises.writeFile(path, "");
			}
