...
class LocalFS extends AFSProvider<LocalFSStats> {
		...
		if (
			...
			if (realPath.startsWith("/")) {
				if (realPath.startsWith(this.domain)) {
					...
					let handle = await red.fs.whatwgfs.getFolder();
	...
	static async newOPFS(redPath: string) {
		const dirHandle = await navigator.storage.getDirectory();

		try {
			await red.fs.promises.mkdir(redPath);
		} catch (e) {
			// Ignore, the directory already exists so we don't need to create it
		}
		const fs = new LocalFS(dirHandle, redPath);
		red?.fs.installProvider(fs);
		const textde = new TextDecoder();
		try {
			fs.stats = new Map(
				JSON.parse(
					textde.decode(
						await fs.promises.readFile(redPath + "/.red_stats"),
					),
				),
			);
		} catch (e: any) {
	...
	static async new(redPath: string) {
		let dirHandle;
		try {
			dirHandle = await window.showDirectoryPicker({
				id: `red-${redPath.replace(/\/|\s|\./g, "-")}`,
			});
		} catch (e) {
			if (e.name !== "TypeError") {
				throw e;
	...
	promises = {
		saveStats: async () => {
			...
			await this.promises.writeFile(this.domain + "/.red_stats", jsonStats);
		},
		writeFile: async (
			path: string,
