...
class RedUI {
	...
	>(name: string): Promise<Component<TProps, TPrivate, TPublic>> {
		...
		const library = await red.import(lib);
	 ...
	 *
	 * ```jsx
	 * const { Button } = await red.ui.use("Button");
	 *
	 ...
	 *
	 * ```js
	 * const { Button, } = await red.ui.use(["Button", "Input"]);
	 *
	 ...
	 */
	init() {
		const components = red.settings.get("red.ui.components");
		...
		const RedVersion: Component<{ product: string }> = function () {
			this.product ||= "Red";
			return (
				<span>
					{this.product} version: {red.version.pretty}
				</span>
			);
		};
		...
		this.registerComponent("RedVersion", RedVersion);
