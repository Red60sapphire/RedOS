...
async function InitV86Hdd(): Promise<FakeFile> {
	...
	const fakefile = {
		...
		save: async (emulator = red.x86?.emulator) => {
...
class V86Backend {
	...
	openpty(
		...
		if (!red.x86!.ready) {
			onData(
				"\u001b[33mThe red x86 subsystem has not yet booted. Please wait for the notification that it has booted and try again.\u001b[0m",
			);
	...
	v86InternalTCPSocket = class v86InternalTCPSocket extends EventTarget {
		...
		constructor(hostname: string, port: string) {
			...
			(async () => {
				this.pty = red.x86!.ptyNum++;
				await red.x86!.sendWispFrame({
					type: "CONNECT",
					streamType: 0x01,
					streamID: this.pty,
					port: port,
		...
		close() {
			red.x86!.closepty(this.pty);
		}

		send(data: any) {
			red.x86!.writeBinaryPTY(this.pty, data);
		}
	...
	async startMouseDriver() {
		...
		const pty = await this.openpty(
			"/bin/ash -c 'env DISPLAY=:0 /bin/redmouse'",
			100,
			100,
			(data) => {
		...
		function write_uint(i: number, addr: number) {
			...
			red.x86!.emulator.write_memory(bytes, addr);
		}
	...
	async twispinit() {
		...
		function sendPacket(
			...
			if (connections[streamID].congestion > 0) {
				...
				red.x86!.emulator.bus.send("virtio-console0-input-bytes", data);
			} else {
		...
		function processIncomingWispFrame(frame: Uint8Array) {
			...
			switch (frame[0]) {
				...
				case 5: // PROTOCOL EXT
					...
					//console.log(full);
					red.x86!.emulator.bus.send("virtio-console0-input-bytes", full);
					red.x86!.onboot();
					break;
			}
		}
