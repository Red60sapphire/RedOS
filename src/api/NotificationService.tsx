class NotificationService {
	...
	add(params: NotifParams) {
		const notif = new RedNotification(params, () => {
			this.remove(notif);
		});
	...
	remove(notification: RedNotification, rendererOnly = false) {
		if (!rendererOnly) {
	...
	subscribe(callback: (notifications: RedNotification[]) => void) {
		let active = true;
...
class RedNotification implements NotifParams {
	title = "Red Notification";
	description = "Red Description";
	timeout: number | "never" = 2000;
	closeIndicator = false;
	callback = (_notif: RedNotification) => null;
	buttons: Array<{
		text: string;
		style?: "contained" | "outlined" | "text";
		callback: (notif: RedNotification) => void;
