class Settings {
	...
	fs: RedFilesystem;
	private constructor(fs: RedFilesystem, inital: { [key: string]: any }) {
		this.fs = fs;
		this.cache = inital;
	...
	async remove(prop: string, subprop?: string) {
		console.warn(
			"red.settings.remove() is a debug feature, and should not be used outside of development.",
		);
		if (subprop) {
