...
class WMWindow extends EventTarget implements Process {
	...
	constructor(
		...
		this.clampWindows = !!red.settings.get("clampWindows");
		...
		this.element = (
			<div
				...
				style={`
                    ...
                    ${red.platform.type === "mobile" ? "top: 5px!important; left: 5px!important;" : ""}
                `}
				on:pointerover={() => {
					this.mouseover = true;
				}}
		...
		for (let i = 0; i < resizers.length; i++) {
			...
			currentResizer.addEventListener("pointerdown", (e: PointerEvent) => {
				e.preventDefault();
				if (red.platform.type === "mobile") return;
		...
		if (!red.settings.get("disable-animation"))
		...
		this.pid = red.processes.state.procs.length;
		red.processes.register(this);
	}
	...
	async remaximize() {
		...
		const animx =
			...
			!red.settings.get("disable-animation");
