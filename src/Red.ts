class Red {
	version = {
		...
		codename: "unknown",
		get pretty() {
			const semantic = red.version.semantic;
			return `${semantic.major}.${semantic.minor}.${semantic.patch}${red.version.buildstate == "stable" ? "" : `-${red.version.buildstate}`}`;
		},
	};
	initComplete = false;
	x86: null | V86Backend;
	...
	static async new(config: any): Promise<Red> {
		...
		const fs = new RedFilesystem([fsProvider]);
		...
		const redPartial = new Red(fs, settings, config, hdd, net);
		return redPartial;
	}
	...
	libs: any = {};
	logger = {
		log: console.log.bind(console, "redOS:"),
		debug: console.debug.bind(console, "redOS:"),
		info: console.info.bind(console, "redOS:"),
		warn: console.warn.bind(console, "redOS:"),
		error: console.error.bind(console, "redOS:"),
			...
			return {
				stdout: new WritableStream({
					write: (message) => {
						...
						console.log(`redOS: ${prefix ? `[${prefix}] ` : ""}${message}`);
					},
				}),
				...
				stderr: new WritableStream({
					write: (message) => {
						...
						console.error(`redOS: ${prefix ? `[${prefix}] ` : ""}${message}`);
					},
				}),
			};
		},
...
await red.registerApp(app); // This will let us capture error messages
		...
		const handlers = red.settings.get("ExternalAppHandlers");
		...
		const handlerModule = await red.import(handler);
		if (!handlerModule) {
	...
	registerExternalAppHandler(id: string, handler: string) {
		const handlers = red.settings.get("ExternalAppHandlers") || {};
		handlers[handler] = id;
		red.settings.set("ExternalAppHandlers", handlers);
	}
	...
	async registerExternalLib(source: string): Promise<ExternalLib> {
		...
		await red.registerLib(lib); // This will let us capture error messages
		return lib;
	}
	removeStaleApps() {
		for (const appName in red.apps) {
			const app = red.apps[appName];
	...
	async import(packageName: string, searchPath?: string) {
		if (searchPath) {
			...
			if (!filename || filename === "") {
				const data: any = await red.fs.promises.readFile(
			...
			const file = await red.fs.promises.readFile(
