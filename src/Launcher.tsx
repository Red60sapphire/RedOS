class Launcher {
	...
	private popupTransition = red.settings.get("disable-animation")
		? "opacity 0.15s"
		: "all 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95)";

	private gridTransition = red.settings.get("disable-animation")
		? "all 0s"
	...
	async init() {
		...
		this.element = (
			<Panel
				id="launcher"
				width={red.platform.type === "mobile" ? "100%" : "min(70%, 35em)"}
				height={use(this.state.active, (active) =>
					active
						? red.platform.type === "mobile"
							? "calc(100% - 75px)"
							: "min(80%, 40em)"
						: red.platform.type == "mobile"
							? "calc(100% - 75px)"
...
> = function () {
	...
	const contextmenu = new red.ContextMenu(true);
	...
	if (red.settings.get("applist").includes(app.package)) {
		contextmenu.addItem(
			...
			function () {
				...
				document.dispatchEvent(new Event("red-force-taskbar-update"));
			},
			"keep_off",
		);
	} else {
	...
	contextmenu.addItem(
		...
		async () => {
			if (
				red.apps[app.package].source &&
				red.apps[app.package].source.includes("/fs")
			) {
				try {
