...
class QuickSettings {
	...
	open() {
		...
		for (let i = 0; i < pinnedSettings.length; i++) {
			pinnedSettings[i]!.value = red.settings.get(
	...
	async init() {
				...
				<div class={["quickSettingsContent"]}>
					<div class={["topButtons"]}>
						<button
							...
							on:click={() => {
								window.location.replace(
									red.settings.get("exitUrl") || "https://google.com/",
								);
							}}
						>
						...
						<button
							...
							on:click={() => {
								try {
									if (red.platform.state.fullscreen) {
						...
						>
							<span class={["material-symbols-outlined"]}>
								{use(red.platform.state.fullscreen, (fullscreen) =>
								...
								settingsElements.push(
									<div
										...
										on:click={() => {
											...
											this.state.pinnedSettings[i]!.value = red.settings.get(
												this.state.pinnedSettings[i]!.registry,
											);
											this.state.pinnedSettings = pinnedSettings;
		...
		this.notificationCenterElement = (
			...
			>
				<div class={["notificationContainer"]}>
					{use(red.notifications.state.notifications, (notifications) =>
				...
				<div class={["clearButtonContainer"]}>
					<button
						...
						on:click={() => {
							red.notifications.state.notifications.forEach((n) => {
								n.close();
							});
							red.notifications.state.notifications = [];
						}}
					>
						Clear all
					</button>
		...
		((this.state.pinnedSettings = red.settings.get("pinnedSettings") || [
			{
		...
		//     red.settings.set("pinnedSettings", pinnedSettings);
		//     pinnedSettings.forEach((setting) => {
		//         red.settings.set(setting.registry, setting.value);
		//     });
		// });

		useChange(use(this.state.showQuickSettings), (show: boolean) => {
			this.updateClickoffChecker(show);
			red.notifications.setRender(!show);
		});
	}
}
...
class QuickSettingsNotification {
	state: Stateful<{
		...
		buttons: Array<{
			...
			callback: (notif: RedNotification) => void;
			close?: boolean;
		}>;
	}> = $state({
		title: "",
	...
	originalNotification: RedNotification;

	constructor(notif: RedNotification) {
		this.state.title = notif.title || "Red Notification";
	...
	close() {
		this.element.remove();
		red.notifications.remove(this.originalNotification);
	}
}
